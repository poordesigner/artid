...
class SupportContextBuilder
{
    ...
    public static function build(string $topic): string
    {
        ...
        $lines = [
            ...
            'No existe soporte humano directo en este chat. Si no puedes resolver un caso (facturacion, tecnico u otro), invita al usuario a revisar la pagina de ayuda https://qrte.poordesigner.com/ayuda y, si lo desea, a crear un ticket de soporte en https://qrte.poordesigner.com/tickets. Si pide hablar con una persona, invítalo a escribir a qart@poordesigner.com con su solicitud.',
            ...
            'Bucle sin avance: si el usuario insiste 3 veces o mas con lo mismo sin avanzar, cierra el hilo con cortesia: sugiere revisar la ayuda en https://qrte.poordesigner.com/ayuda y, si lo necesita, crear un ticket de soporte en https://qrte.poordesigner.com/tickets.',
    ...
    private static function packLines(string $topic): array
    {
        ...
        return array_map(
            fn (string $line) => match (true) {
                str_contains($line, '{welcome_tokens}') => str_replace('{welcome_tokens}', (string) (int) config('qrte.welcome_tokens', 0), $line),
