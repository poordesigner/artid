<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        <!-- Signature image -->
        <div>
            <x-input-label for="signature_image" :value="__('Firma manuscrita (para COA)')" />
            @if ($user->signature_image)
                <img src="{{ $user->signatureUrl() }}" alt="Firma" class="mt-2 h-16 w-auto bg-white border border-gray-200 p-2 rounded" />
                <label class="mt-2 flex items-center gap-2 text-xs text-gray-600">
                    <input type="checkbox" name="remove_signature" value="1" class="rounded border-gray-300 text-indigo-600"> {{ __('Eliminar firma') }}
                </label>
            @endif
            <input id="signature_image" name="signature_image" type="file" accept="image/png,image/jpeg" class="block mt-2 w-full text-sm text-gray-700 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            <p class="mt-1 text-xs text-gray-500">{{ __('Opcional. PNG/JPG sobre fondo blanco, máx 2 MB. La eliges al generar cada COA.') }}</p>
