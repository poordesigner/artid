...
<head>
    ...
    <title>{{ __('Artistas como vos') }}</title>
    <style>
        ...
        .stat { display: inline-block; margin: 16px 0; padding: 20px 32px; background: #eef2ff; border-radius: 8px; text-align: center; }
        .stat .number { font-size: 36px; font-weight: 800; color: #4f46e5; display: block; }
...
<body>
    <div class="wrap">
        ...
        <div class="body">
            <div class="msg">
                <p>{{ __('Ya creaste tu primera obra. ¡Felicitaciones!') }}</p>
                <p>{{ __('Artistas como vos ya están usando QRTE para darle identidad digital a sus obras.') }}</p>
                <div class="stat">
                    <span class="number">{{ $artistCount }}</span>
                    <span class="label">{{ __('artistas ya registrados en QRTE') }}</span>
                </div>
                <p>{{ __('Seguí creando obras para expandir tu portafolio digital. Cada obra obtiene su propio QR y ficha pública verificada.') }}</p>
                <a class="btn" href="{{ url('/artworks') }}">{{ __('Ver mis obras') }}</a>
            </div>
        </div>
        <div class="foot">
