@php $locked = isset($artwork) && $artwork && $artwork->isQrValidated(); @endphp
@if($locked)
<div class="mb-4 p-3 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">QR validado — Título, Imagen, Año, Edición, Nº Copias y Dimensiones están bloqueados.</div>
@endif
<!-- Title -->
<div>
    <x-input-label for="title" :value="__('Title')" />
    <x-text-input id="title" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="text" name="title" :value="old('title', $artwork->title ?? '')" required autofocus :disabled="$locked" />
...
<div class="mt-4">
    ...
    <input id="image" name="image" type="file" accept="image/*" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full text-sm text-gray-700 {{ $locked ? 'bg-gray-100 opacity-60' : '' }}" @disabled($locked) />
    <p class="mt-1 text-xs text-gray-500">{{ $locked ? __('Bloqueado tras validar QR.') : __('Imagen JPG o PNG, máximo 2 MB.') }}</p>
...
<div class="mt-4">
    ...
    <x-text-input id="year" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="number" name="year" min="1000" :max="date('Y') + 1" :value="old('year', $artwork->year ?? '')" placeholder="Ej: 2025" :disabled="$locked" />
...
<div class="mt-4" x-data="editionPicker()">
    ...
    <select id="edition_type" name="edition_type" x-model="type" @change="updateEdition()" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" @disabled($locked)>
    ...
    <div x-show="type === 'edition'" x-transition class="mt-3">
        ...
        <x-text-input id="edition_copies" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="number" x-model="copies" min="1" @input="updateEdition()" :value="old('edition_copies')" :disabled="$locked" />
    </div>
...
<div class="mt-4" x-data="dimensionsPicker()">
    ...
    <div class="mt-1 grid grid-cols-3 gap-3">
        <div>
            ...
            <x-text-input id="dim_height" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="number" step="0.1" x-model="height" @input="updateDimensions()" placeholder="0" :disabled="$locked" />
        </div>
        <div>
            <x-input-label for="dim_width" :value="__('Ancho')" />
            <x-text-input id="dim_width" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="number" step="0.1" x-model="width" @input="updateDimensions()" placeholder="0" :disabled="$locked" />
        </div>
        <div>
            <x-input-label for="dim_depth" :value="__('Profundidad')" />
            <x-text-input id="dim_depth" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="number" step="0.1" x-model="depth" @input="updateDimensions()" placeholder="0" :disabled="$locked" />
        </div>
    </div>
    <div class="mt-3">
        <x-input-label for="dim_unit" :value="__('Unidad')" />
        <select id="dim_unit" name="dim_unit" x-model="unit" @change="updateDimensions()" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" @disabled($locked)>
...
<!-- Creation location -->
<div class="mt-4">
    <x-input-label for="creation_location" :value="__('Lugar de creación (opcional)')" />
    <x-text-input id="creation_location" list="locations" class="block mt-1 w-full" type="text" name="creation_location" :value="old('creation_location', $artwork->creation_location ?? '')" placeholder="Ej: Medellín, Colombia" />
    <x-locations-datalist />
    <x-input-error :messages="$errors->get('creation_location')" class="mt-2" />
</div>

<!-- Edition number (for tiraje) -->
<div class="mt-4" x-data="{ type: document.getElementById('edition_type')?.value || 'single' }" x-init="$watch(() => document.getElementById('edition_type')?.value, v => type = v)">
    <div x-show="type === 'edition'" x-transition>
        <x-input-label for="edition_number" :value="__('Nº de ejemplar (opcional)')" />
        <x-text-input id="edition_number" class="block mt-1 w-full {{ $locked ? 'bg-gray-100' : '' }}" type="number" name="edition_number" min="1" :value="old('edition_number', $artwork->edition_number ?? '')" placeholder="Ej: 2" :disabled="$locked" />
        <p class="mt-1 text-xs text-gray-500">{{ $locked ? __('Bloqueado tras validar QR.') : __('Ej. 2 para 2/5. Se muestra como 2/5 en ficha y COA.') }}</p>
